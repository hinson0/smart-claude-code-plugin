...
└── plugins/smart/
    ...
    └── skills/
        ...
        ├── <name>/CN.md             # 同目录中文翻译
        └── …（advance-one-step / close-issue / commit / distill / help / hud / learning / local / notebook / optimize-tokens / sendshot / show / todo / wfb）
```
...
- **安全收口**：close-issue 默认只读；写 note 与关闭 GitLab Issue 必须有明确授权，且不扩展为 push、merge、MR/PR、checklist 或标签权限。
