...
function parseArgs(argv) {
  ...
  const issue = /^#\d+$/.test(values["--issue"])
    ? values["--issue"].slice(1)
    : values["--issue"];
...
async function main() {
  ...
  const warnings = localOnlyException
    ? [
        `${options.remote}/${options.targetBranch} does not contain the implementation commit; continuing under the user's explicit exception`,
      ]
    : [];
  const noteResult = run(
    ...
    "Development-asset note publish",
  );
  if (!noteResult.ok) {
    emit(
      {
