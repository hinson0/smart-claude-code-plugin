...
description: 当用户说"analyze"、"优化插件"、"禁用无用插件"、"节省上下文"、"减少上下文占用"、"我需要哪些插件"、"清理插件"、"聚焦上下文"、"精简插件"，或想要检测项目类型并禁用无关插件以节省上下文窗口空间时，应使用此技能。通过 /smart:optimize-tokens 显式调用。
argument-hint: 无需参数。自动检测项目类型并推荐插件变更。
---

# optimize-tokens — 上下文感知插件优化器
...
- 已禁用插件（数量和名称）
- 已保留插件（数量）
- 提醒："切换项目后可再次运行 `/smart:optimize-tokens` 重新优化。"
